...
def main():
    ...
    try:
        ...
        with TextUserInterface(sys.stderr) as tui:
            if args.init:
                size = tui.terminal_size()
                str_size = "{}x{}".format(size.lines, size.columns)

                print()
                print("Press Q Q Q to quit {}, or other keys", str_size, end="\r\n")
                print()

                # fits inside macOS 5x20, Safari Gcp 4x53, Chrome Gcp 10x50
...
# posted as:  https://github.com/pelavarre/byoverbs/blob/main/demos/glass1.py
